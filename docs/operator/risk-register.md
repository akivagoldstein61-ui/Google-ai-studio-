...
|---|---:|---|---|---|
| Prototype auth mode can allow unauthenticated AI routes | High | `AI_ROUTE_AUTH_MODE` defaults to `prototype` in `server/aiRoutes.ts` | Documented in `AGENTS.md`; strict mode exists | Set strict mode in production and verify Firebase Admin init |
...
| Admin screens are present without proven role gating | High | `AIOpsScreen`, `ExperimentsScreen`, settings navigation | Feature is prototype-only | Gate by admin role before production |
