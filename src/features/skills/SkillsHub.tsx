...
/**
 * Skills that map to a real, working feature surface in the app.
 * Tapping "Open the live feature" navigates the user straight into it,
 * so the skill is demonstrably functioning — not just an explainer page.
 */
export const SKILL_LIVE_ROUTES: Record<string, { path: string; label: string }> = {
  'personality-assessment': { path: '/settings/personality', label: 'Open assessment' },
  'personality-profile': { path: '/settings/personality', label: 'Open your profile' },
  'personality-visibility': { path: '/settings/personality-visibility', label: 'Open visibility settings' },
  'private-taste': { path: '/settings/taste-profile', label: 'Open taste profile' },
  'consent-ux': { path: '/settings/ai-trust', label: 'Open AI & Trust Hub' },
  'explainable-ai': { path: '/settings/ai-trust', label: 'Open AI & Trust Hub' },
  'why-this-match': { path: '/daily', label: 'See it on Daily Picks' },
  'pacing-coach': { path: '/daily', label: 'See it on Daily Picks' },
  'compatibility-reflection': { path: '/inbox', label: 'Open Matches' },
  'permissioned-sharing': { path: '/inbox', label: 'Open Matches' },
};
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((skill) => {
            ...
            const live = SKILL_LIVE_ROUTES[skill.id];
            return (
              <div
                key={skill.id}
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  ...
                  {live && onOpenFeature && (
                    <button
                      onClick={() => onOpenFeature(live.path)}
                    ...
                    >
                      {live.label} <ChevronRight size={11} />
                    </button>
                  )}
                </div>
              </div>
